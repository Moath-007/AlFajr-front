...
export const productsService = {
  ...
  listRetail: (query: CatalogProductsQuery = {}, signal?: AbortSignal) =>
    apiClient.get<CatalogProductsListResponseDto>(
      withCatalogQuery("/products/retail", query),
      { signal },
    ),
  getRetailById: (id: number, signal?: AbortSignal) =>
    apiClient.get<CatalogProductDetailsResponseDto>(`/products/retail/${id}`, {
      signal,
    }),
  listWholesale: (query: CatalogProductsQuery = {}, signal?: AbortSignal) =>
    apiClient.get<CatalogProductsListResponseDto>(
      withCatalogQuery("/products/wholesale", query),
      { signal },
    ),
  getWholesaleById: (id: number, signal?: AbortSignal) =>
    apiClient.get<CatalogProductDetailsResponseDto>(
      `/products/wholesale/${id}`,
      { signal },
    ),
  create: (data: CreateProductDto, signal?: AbortSignal) =>
    apiClient.post<CreateProductResponseDto>(
      "/products",
      toCreateProductFormData(data),
      { signal },
    ),
  update: (id: number, data: UpdateProductDto, signal?: AbortSignal) =>
    apiClient.put<UpdateProductResponseDto>(`/products/${id}`, data, {
      signal,
    }),
  updateImages: (
    productId: number,
  ...
  updateStatus: (
    ...
    apiClient.patch<UpdateProductStatusResponseDto>(
      `/products/${id}/status`,
      data,
      { signal },
    ),
...
function withAdminQuery(path: string, query: AdminProductsQuery): string {
...
function withCatalogQuery(path: string, query: CatalogProductsQuery): string {
  ...
  if (query.color_id !== undefined)
    params.set("color_id", String(query.color_id));
...
function toUpdateProductImagesFormData(data: UpdateProductImagesDto): FormData {
  ...
  if (data.primaryExistingImageId !== undefined) {
    formData.append(
      ...
      String(data.primaryExistingImageId),
    );
  }

...
export default function AboutPage() {
  ...
  const companyName = company?.company_name?.trim() || 'شركة الفجر للصناعة والتجارة';

  return (
    <div className="min-h-screen bg-white">
      <section className="relative overflow-hidden bg-[#162E21] text-white">
        <div className="absolute -left-32 -top-32 h-96 w-96 rounded-full border-[64px] border-[#9C7537]/10" />
        <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <p className="text-sm font-extrabold text-[#D8B16D]">تعرف علينا عن قرب</p>
          ...
          <p className="mt-5 max-w-2xl text-base leading-8 text-stone-300 sm:text-lg">
            {companyName} متخصصة في منتجات وتجهيزات الحمامات، وتقدم خيارات متنوعة تناسب احتياجات المساحات المختلفة.
          </p>
        </div>
      </section>
      ...
      <main>
        <section className="py-16 sm:py-24">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-start lg:px-8">
            <div>
              <p className="text-sm font-extrabold text-[#9C7537]">عن الفجر</p>
        ...
        <section className="border-y border-stone-200 bg-stone-50/80 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="max-w-2xl">
              <p className="text-sm font-extrabold text-[#9C7537]">ما الذي نقدمه؟</p>
            ...
            <div className="mt-9 grid gap-4 md:grid-cols-3">
              {offerings.map((offering) => (
                <article key={offering.title} className="group relative min-h-52 overflow-hidden rounded-2xl border border-[#162E21]/10 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-[#9C7537]/50 hover:shadow-xl hover:shadow-[#162E21]/5 sm:p-7">
                  <span className="absolute inset-x-0 top-0 h-1 origin-right scale-x-0 bg-[#9C7537] transition-transform duration-300 group-hover:scale-x-100" />
                  <span className="block h-px w-12 bg-[#9C7537]/60 transition-all duration-300 group-hover:w-20" />
        ...
        <section className="py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid overflow-hidden rounded-[2rem] bg-[#162E21] text-white lg:grid-cols-[0.8fr_1.2fr]">
              <div className="bg-[#9C7537] p-8 sm:p-10"><p className="text-sm font-extrabold text-white/75">طريقة عملنا</p><h2 className="mt-3 text-3xl font-black leading-tight">وضوح في العرض واهتمام بالاختيار</h2></div>

  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row">
        ...
        <button onClick={() => setShowFilters(true)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm font-bold text-[#162E21] lg:hidden"><SlidersHorizontal className="h-4 w-4" /> الفلاتر {activeFilters > 0 && <span className="rounded-full bg-[#9C7537] px-2 py-0.5 text-xs text-white">{activeFilters}</span>}</button>
      </div>

      <div className="grid gap-7 lg:grid-cols-[250px_1fr]">
        <aside className="hidden lg:block"><div className="sticky top-40 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm"><div className="mb-5 flex items-center justify-between"><h2 className="font-black text-[#162E21]">تصفية النتائج</h2>{activeFilters > 0 && <button onClick={clearFilters} className="text-xs font-bold text-[#9C7537]">مسح الكل</button>}</div>{filters}</div></aside>
        <section>
...
function PageButton({ children, disabled, onClick }: { children: React.ReactNode; disabled: boolean; onClick: () => void }) { return <button disabled={disabled} onClick={onClick} className="inline-flex h-10 items-center gap-1 rounded-xl border border-stone-200 bg-white px-3 text-sm font-bold text-stone-600 transition hover:border-[#9C7537] disabled:cursor-not-allowed disabled:opacity-40">{children}</button>; }
...
function formatPrice(value: string): string { const parsed = Number(value); return Number.isFinite(parsed) ? `${new Intl.NumberFormat('ar', { maximumFractionDigits: 2 }).format(parsed)} ₪` : 'السعر غير متاح'; }

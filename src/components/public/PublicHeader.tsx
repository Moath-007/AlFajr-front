      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-3 lg:h-[68px]">
          <button onClick={() => onNavigate('home')} className="group flex min-w-0 shrink-0 items-center gap-3 text-right">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#162E21] text-lg font-black text-[#E8DCC2] shadow-sm transition-transform group-hover:-translate-y-0.5">ف</span>
          ...
          <nav className="hidden items-center gap-1 lg:flex" aria-label="التنقل الرئيسي">
            {navItems.map((item) => (
              <button key={item.id} onClick={() => onNavigate(item.id)} aria-current={isActive(item.path) ? 'page' : undefined} className={`relative rounded-xl px-3.5 py-2.5 text-sm font-bold transition-colors ${isActive(item.path) ? 'bg-[#162E21]/[0.07] text-[#162E21]' : 'text-stone-600 hover:bg-stone-100 hover:text-[#162E21]'}`}>
                {item.label}
                {isActive(item.path) && <span className="absolute inset-x-3 -bottom-[11px] h-0.5 rounded-full bg-[#C2A66D]" />}
              </button>
            ))}
          </nav>

          <div className="flex items-center gap-1.5 sm:gap-2">
            <button onClick={() => onNavigate('login')} className="hidden min-h-10 items-center gap-1.5 rounded-xl border border-[#C2A66D]/45 bg-[#C2A66D]/[0.07] px-3.5 text-sm font-extrabold text-[#162E21] transition hover:border-[#C2A66D] hover:bg-[#C2A66D]/15 focus:outline-none focus:ring-2 focus:ring-[#C2A66D]/30 lg:inline-flex"><LogIn className="h-4 w-4 text-[#C2A66D]" /> تسجيل الدخول</button>
            <button onClick={() => onNavigate('cart')} className="relative rounded-xl p-2.5 text-stone-600 transition hover:bg-stone-100 hover:text-[#162E21] focus:outline-none focus:ring-2 focus:ring-[#C2A66D]/40" aria-label={`السلة${totalQuantity ? `، ${totalQuantity} قطع` : ''}`}><ShoppingBag className="h-5 w-5" />{totalQuantity > 0 && <span className="absolute -right-1 -top-1 grid min-h-5 min-w-5 place-items-center rounded-full bg-[#C2A66D] px-1 text-[10px] font-black text-[#162E21]">{totalQuantity}</span>}</button>
            <button onClick={() => setSearchOpen((open) => !open)} className="rounded-xl p-2.5 text-stone-600 transition hover:bg-stone-100 hover:text-[#162E21] focus:outline-none focus:ring-2 focus:ring-[#C2A66D]/40" aria-label="فتح البحث" aria-expanded={searchOpen}><Search className="h-5 w-5" /></button>
            <button onClick={() => setMobileOpen((open) => !open)} className="rounded-xl p-2.5 text-stone-700 transition hover:bg-stone-100 focus:outline-none focus:ring-2 focus:ring-[#C2A66D]/40 lg:hidden" aria-label="فتح القائمة" aria-expanded={mobileOpen}>{mobileOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button>
          </div>
        </div>

...
interface PublicNotFoundPageProps {
  onNavigate: (page: string) => void;
}

export default function PublicNotFoundPage({ onNavigate }: PublicNotFoundPageProps) {
  return (
    <main className="mx-auto grid min-h-[62vh] max-w-2xl place-items-center px-4 py-16 text-center sm:px-6">
      <div className="w-full rounded-3xl border border-stone-200 bg-white px-6 py-10 shadow-sm sm:px-10 sm:py-12">
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          <button onClick={() => onNavigate('home')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#162E21] px-5 py-2.5 text-sm font-black text-white transition hover:bg-[#21452f] focus:outline-none focus:ring-4 focus:ring-[#162E21]/15"><Home className="h-4 w-4" /> العودة للرئيسية</button>
          <button onClick={() => onNavigate('products')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-stone-200 bg-white px-5 py-2.5 text-sm font-bold text-stone-700 transition hover:border-[#C2A66D]/60 hover:text-[#162E21] focus:outline-none focus:ring-4 focus:ring-[#C2A66D]/10">تصفح المنتجات <ArrowLeft className="h-4 w-4" /></button>
        </div>
      </div>
    </main>
  );

...
export default function CartPage({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-9 lg:px-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-extrabold text-[#C2A66D]">مراجعة المنتجات</p><h1 className="mt-1 text-2xl font-black text-[#162E21] sm:text-[1.75rem]">سلة المشتريات</h1></div><button onClick={clearCart} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold text-red-700 transition hover:bg-red-50"><Trash2 className="h-4 w-4" /> تفريغ السلة</button></div>
      <div className="grid gap-5 lg:grid-cols-[1fr_310px] lg:items-start">
        <section className="space-y-3" aria-label="منتجات السلة">
          {items.map((item) => <article key={item.product_variant_id} className="grid gap-3 rounded-2xl border border-stone-200 bg-white p-3 shadow-sm sm:grid-cols-[88px_1fr_auto] sm:items-center">
            ...
            <div><h2 className="text-base font-black text-[#162E21]">{item.product_name}</h2><p className="mt-0.5 text-xs font-bold text-stone-400">الكود: {item.product_code}</p><p className="mt-1.5 text-sm font-semibold text-stone-600">{item.size} — {item.color}</p><div className="mt-2 flex flex-wrap items-baseline gap-2">{item.display_discount > 0 && <><span className="text-xs font-bold text-stone-400 line-through">{formatPrice(item.display_price)}</span><span className="rounded-md bg-[#C2A66D]/10 px-1.5 py-0.5 text-[11px] font-black text-[#C2A66D]">خصم {formatPrice(item.display_discount)}</span></>}<span className="font-black text-[#C2A66D]">{formatPrice(item.display_effective_price)}</span></div></div>

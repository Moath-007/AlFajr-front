    ...
    <div className="overflow-hidden bg-white">
      ...
      <section className="border-t border-[#162E21]/10 bg-[#E9EDE9] px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <div className="relative mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 overflow-hidden rounded-2xl border border-[#162E21]/15 bg-gradient-to-l from-[#F7F5EF] via-[#F3F5F1] to-[#E5ECE6] px-6 py-8 shadow-[0_12px_35px_rgba(22,46,33,0.08)] sm:px-8 md:flex-row md:items-center lg:px-10">
          ...
          <div className="relative flex w-full flex-col gap-3 sm:w-auto sm:flex-row"><button onClick={() => onNavigate('products')} className="rounded-xl bg-[#162E21] px-6 py-3 text-sm font-extrabold text-white transition hover:bg-[#21452f]">تصفح المنتجات</button><button onClick={() => onNavigate('contact')} className="rounded-xl border border-[#C2A66D]/50 bg-[#C2A66D]/[0.06] px-6 py-3 text-sm font-extrabold text-[#162E21] transition hover:border-[#C2A66D] hover:bg-[#C2A66D]/15">تواصل معنا</button></div>
        </div>
      </section>
    </div>
  );
...
function SectionHeading({ eyebrow, title, description, actionLabel, onAction }: { eyebrow: string; title: string; description: string; actionLabel: string; onAction: () => void }) {
  return <div className="flex items-end justify-between gap-6"><div><p className="text-sm font-extrabold text-[#C2A66D]">{eyebrow}</p><h2 className="mt-2 text-2xl font-black text-[#162E21] sm:text-3xl">{title}</h2><p className="mt-2 text-sm leading-6 text-stone-500 sm:text-base">{description}</p></div><button onClick={onAction} className="hidden shrink-0 items-center gap-2 text-sm font-extrabold text-[#162E21] transition hover:text-[#C2A66D] sm:inline-flex">{actionLabel}<ArrowLeft className="h-4 w-4" /></button></div>;
}

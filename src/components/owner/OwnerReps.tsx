...
import { Plus, Pencil, Eye, UserCheck, UserX, Users, Search, Phone, Mail } from 'lucide-react';
...
export default function OwnerReps({ reps, orders, onAdd, onUpdate, onToggle, onNotify }: OwnerRepsProps) {
  ...
  const [form, setForm] = useState({ name: '', phone: '', username: '', password: '', email: '', active: true });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const filtered = useMemo(() => {
    return reps.filter((r) => !search || r.name.includes(search) || r.username.includes(search) || r.phone.includes(search));
  }, [reps, search]);

  const openAdd = () => {
    setEditing(null);
    setForm({ name: '', phone: '', username: '', password: '', email: '', active: true });
    setErrors({});
    setModalOpen(true);
  };
  ...
  const openEdit = (r: Representative) => {
    setEditing(r);
    setForm({ name: r.name, phone: r.phone, username: r.username, password: r.password, email: r.email, active: r.active });
    setErrors({});
    setModalOpen(true);
  };
  ...
  const validate = () => {
    ...
    else if (!/^0\d{9,10}$/.test(form.phone.replace(/\s/g, ''))) e.phone = 'رقم الهاتف غير صحيح';
  ...
  const repOrderCount = (repId: string) => orders.filter((o) => o.repId === repId).length;
      ...
      <div className="space-y-8 animate-fade-in pb-16">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-stone-200/80 shadow-xs flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-stone-900 tracking-tight">إدارة المناديب</h1>
            <p className="text-stone-500 mt-1 text-sm font-medium">عرض ومتابعة مناديب المبيعات ({reps.length} مندوب مسجل)</p>
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tr>
                    ...
                    <th className="px-5 py-4 text-right font-bold">عدد الطلبات</th>
                      ...
                      <tr key={r.id} className="hover:bg-stone-50/80 transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-black text-stone-900">{r.name}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-stone-600 font-bold" dir="ltr">{r.phone}</td>
                        <td className="px-5 py-4 text-stone-600 font-bold" dir="ltr">{r.username}</td>
                        <td className="px-5 py-4 text-stone-500 text-xs" dir="ltr">{r.email}</td>
                        <td className="px-5 py-4">
                          {r.active ? <span className="badge-green">نشط</span> : <span className="badge-gray">غير نشط</span>}
                        </td>
                        <td className="px-5 py-4 font-black text-amber-600">{repOrderCount(r.id)} طلبات</td>
                        <td className="px-5 py-4 text-center">
                          <div className="flex items-center justify-center gap-1.5 bg-stone-50 p-1 rounded-xl border border-stone-200/60 w-fit mx-auto">
                            <button onClick={() => setViewing(r)} className="p-2 rounded-lg text-blue-600 hover:bg-blue-50 transition-colors" title="عرض">
                              <Eye className="h-4 w-4" />
                            </button>
                            <button onClick={() => openEdit(r)} className="p-2 rounded-lg text-amber-600 hover:bg-amber-50 transition-colors" title="تعديل">
                              <Pencil className="h-4 w-4" />
                            </button>
                            <button
                                onClick={() => { onToggle(r.id); onNotify(r.active ? 'تم تعطيل المندوب' : 'تم تفعيل المندوب', 'info'); }}
        ...
        >
          <form onSubmit={handleSave} className="space-y-5 pt-2">
            <div>
              <label className="block text-xs font-bold text-stone-600 mb-1.5">الاسم الكامل</label>
              <input
                  type="text"
                  value={form.name}
              ...
              />
              {errors.name && <p className="text-xs text-rose-600 mt-1.5 font-bold">{errors.name}</p>}
            </div>

            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-stone-600 mb-1.5">رقم الهاتف</label>
                <input
                    type="tel"
                    value={form.phone}
                    ...
                    dir="ltr"
                />
                {errors.phone && <p className="text-xs text-rose-600 mt-1.5 font-bold">{errors.phone}</p>}
              </div>

              <div>
                <label className="block text-xs font-bold text-stone-600 mb-1.5">البريد الإلكتروني</label>
                <input
                    type="email"
                    value={form.email}
                    ...
                    dir="ltr"
                />
                {errors.email && <p className="text-xs text-rose-600 mt-1.5 font-bold">{errors.email}</p>}
              </div>

              <div>
                <label className="block text-xs font-bold text-stone-600 mb-1.5">اسم المستخدم</label>
                <input
                    type="text"
                    value={form.username}
                    ...
                    dir="ltr"
                />
                {errors.username && <p className="text-xs text-rose-600 mt-1.5 font-bold">{errors.username}</p>}
              </div>

              <div>
                <label className="block text-xs font-bold text-stone-600 mb-1.5">كلمة المرور</label>
                <input
                    type="text"
                    value={form.password}
                    ...
                    dir="ltr"
                />
                {errors.password && <p className="text-xs text-rose-600 mt-1.5 font-bold">{errors.password}</p>}
              </div>
            </div>

            <div className="flex items-center justify-end gap-3 pt-4 border-t border-stone-100">
              <button type="button" onClick={() => setModalOpen(false)} className="px-5 py-3 rounded-2xl text-stone-600 hover:bg-stone-100 text-sm font-bold transition-colors">إلغاء</button>
              <button type="submit" className="btn-gold !py-3 !px-6 rounded-2xl font-bold shadow-sm">{editing ? 'حفظ التعديلات' : 'إضافة المندوب'}</button>
            </div>
          </form>
        </Modal>

        {/* نافذة عرض تفاصيل المندوب الفخمة */}
        <Modal open={!!viewing} onClose={() => setViewing(null)} title="تفاصيل المندوب" size="md">
          {viewing && (
              <div className="space-y-6 pt-2">
                <div className="flex items-center gap-4 bg-stone-50/80 p-5 rounded-2xl border border-stone-200/60">
                  ...
                  <div>
                    <div className="text-xl font-black text-stone-900 mb-1">{viewing.name}</div>
                    <span className={viewing.active ? 'badge-green' : 'badge-gray'}>{viewing.active ? 'حساب نشط' : 'حساب معطل'}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3.5 text-sm">
                  <div className="flex items-center gap-3 p-4 rounded-2xl bg-stone-50/80 border border-stone-200/60">
                    ...
                    <span className="text-stone-700 font-bold" dir="ltr">{viewing.phone}</span>
                  </div>

                  <div className="flex items-center gap-3 p-4 rounded-2xl bg-stone-50/80 border border-stone-200/60">
                    <Mail className="h-5 w-5 text-amber-600" />
                    <span className="text-stone-700 font-bold" dir="ltr">{viewing.email}</span>
                  </div>

                  <div className="p-4 rounded-2xl bg-stone-50/80 border border-stone-200/60 flex items-center justify-between">
                    <span className="text-stone-400 font-bold text-xs">اسم المستخدم:</span>
                    <strong className="text-stone-900 font-black" dir="ltr">{viewing.username}</strong>
                  </div>

                  <div className="p-4 rounded-2xl bg-stone-50/80 border border-stone-200/60 flex items-center justify-between">
                    <span className="text-stone-400 font-bold text-xs">تاريخ الانضمام:</span>
                    <strong className="text-stone-800 font-bold">{viewing.createdAt}</strong>
                  </div>

                  <div className="p-4 rounded-2xl bg-amber-50/60 border border-amber-200/60 flex items-center justify-between">
                    <span className="text-amber-800 font-bold text-xs">إجمالي الطلبات المنفذة:</span>
                    <strong className="text-amber-700 font-black text-base">{repOrderCount(viewing.id)} طلب</strong>
                  </div>
                </div>
              </div>
          )}

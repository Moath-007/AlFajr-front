  ...
  return (
    <div className="space-y-8 animate-fade-in pb-28">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-stone-200/80 shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500 mt-1 text-sm font-medium">
            عرض ومتابعة كافة طلبات الزبائن ({filtered.length} طلب مطابق)
          </p>
        </div>
      </div>
      {/* شريط البحث والفلاتر */}
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 w-[90%] max-w-md">
              <div className="flex items-center justify-between bg-stone-900/95 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 shadow-2xl text-white">
                ...
                <div className="text-xs font-extrabold tracking-wide">
                  صفحة <span className="text-amber-400">{currentPage}</span> من{" "}
          ...
          <div className="space-y-6 pt-2">
            ...
            <div className="grid sm:grid-cols-2 gap-3.5">
              ...
              <div className="bg-stone-50/80 p-4 rounded-2xl border border-stone-200/60">
                ...
                <div className="font-black text-stone-900 text-base" dir="ltr">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-stone-50 font-medium">
                  {viewing.items.map((item, i) => (
                    <tr key={i} className="hover:bg-stone-50/50">
                      ...
                      <td className="px-4 py-3 text-stone-600 font-bold">
                        {item.quantity}
                      </td>
                      <td className="px-4 py-3 text-stone-600">
                        {formatPrice(item.unitPrice)}
                      </td>
                      <td className="px-4 py-3 font-black text-amber-600">
                ...
                <tfoot className="bg-stone-50/80 border-t border-stone-100">
                  <tr>
                    <td
                      colSpan={5}
                      className="px-4 py-3.5 text-left font-black text-stone-900"
                    >
                      إجمالي الطلب الكلي:
                    </td>
                    <td className="px-4 py-3.5 text-lg font-black text-amber-600">
                      {formatPrice(viewing.total)}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-2xl bg-stone-50 border border-stone-200/80">
              <div className="flex items-center gap-2">
                ...
                <span className={getStatusBadgeClass(viewing.status)}>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setStatusChange({ order: viewing, status: "completed" })
                  }
                ...
                <button
                  onClick={() =>
                    setStatusChange({ order: viewing, status: "cancelled" })
                  }
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (statusChange)
            handleStatusChange(statusChange.order, statusChange.status);
          setStatusChange(null);
        }}
        severity={
          statusChange?.status === "cancelled" ? "destructive" : "normal"
        }
        title={
          statusChange?.status === "cancelled" ? "إلغاء الطلب" : "إكمال الطلب"
        }
        message={
          statusChange?.status === "cancelled"
            ? "هل أنت متأكد أنك تريد إلغاء هذا الطلب؟ سيتم اعتبار هذا الإجراء نهائيًا."
            : "هل أنت متأكد أنك تريد تغيير حالة الطلب إلى مكتمل؟"
        }
        confirmLabel={
          statusChange?.status === "cancelled" ? "إلغاء الطلب" : "إكمال الطلب"
        }
